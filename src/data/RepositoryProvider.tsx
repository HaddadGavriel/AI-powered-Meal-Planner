...
export function RepositoryProvider({ children }: { children: React.ReactNode }) {
  ...
  const run = useCallback(
    async <T,>(work: () => Promise<T>, success?: string) => {
      setError(null);
      setMessage(null);
      try {
  ...
  return (
    <Context.Provider value={{ repo, data, user, loading, error, message, refresh, run }}>
      {children}
    </Context.Provider>
  );
}
export function useMealPlanner() {
  ...
  if (!value) throw new Error('RepositoryProvider missing');
  return value;
}
