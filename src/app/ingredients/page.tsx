...
export default function Ingredients() {
  ...
  const can = user?.role !== 'member',
    items = data.ingredients.filter(
      (x) =>
        x.name.toLowerCase().includes(q.toLowerCase()) && (filter === 'all' || x.status === filter),
    );
  return (
    <AppShell>
      ...
      <Card>
        <div className="flex gap-3">
          ...
          {can && (
            <Button className="self-end" onClick={() => setCreate(!create)}>
              Create ingredient
            </Button>
          )}
        </div>
      ...
      {items.length ? (
        <div className="grid gap-3 md:grid-cols-2">
          {items.map((x) => (
            <Card key={x.id}>
              ...
              <p>
                {x.category} · {x.defaultUnit}
              </p>
              <p className="text-sm">Allergens: {x.allergens.join(', ') || 'none declared'}</p>
              {edit === x.id && <IngredientForm item={x} onSaved={() => setEdit(null)} />}{' '}
              {can && (
                <div className="mt-3 flex gap-2">
                  ...
                  >
                    {x.status === 'active' ? 'Archive' : 'Restore'}
                  </Button>
                  <Button
                    variant="destructive"
                    onClick={() => {
        ...
        <Empty title="No ingredients found" body="Clear filters or create the first ingredient." />
      )}
    </AppShell>
  );
}
