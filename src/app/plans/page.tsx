  ...
  return (
    <AppShell>
      ...
      {data.plans.length ? (
                              ...
                              {canManage && (
                                <div className="mt-2 flex flex-wrap gap-1">
                                  <Button
                                    ...
                                    variant="secondary"
                                    onClick={() => setEditMeal(editMeal === entry.id ? null : entry.id)}
                                  >
                                    Edit or move
                                  </Button>
                                  <Button
                                    className="min-h-8 px-2 py-1 text-xs"
                                    variant="destructive"
                                    onClick={() => run(() => repo.removeMeal(plan.id, entry.id), 'Meal removed.')}
                                  >
                                    Remove
                                  </Button>
                                </div>
              ...
              {canManage && (
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      run(
                        () => repo.updatePlan(plan.id, { status: plan.status === 'archived' ? 'draft' : 'archived' }),
                        'Plan status updated.',
                      )
                    }
                  >
                  ...
                  <Button variant="destructive" onClick={() => run(() => repo.deletePlan(plan.id), 'Plan deleted.')}>
                    Delete
                  </Button>
                </div>
              )}
        ...
        <Empty title="No plans" body="Create a weekly plan. Empty plans and recipe libraries are supported." />
      )}
    </AppShell>
  );
}
