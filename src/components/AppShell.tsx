    ...
    return (
      <main className="container-page grid min-h-screen place-items-center">
        <section className="card">
          ...
          <Link className="mt-4 inline-flex font-semibold underline" href="/login">
            Return to sign in
          </Link>
        </section>
      </main>
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-20 border-b border-[rgb(var(--border))] bg-[rgb(var(--surface))]/90 backdrop-blur">
        <div className="container-page flex flex-wrap items-center justify-between gap-3 py-3">
          <Link href="/dashboard" className="flex items-center gap-3 text-lg font-bold">
            <span className="grid size-10 place-items-center rounded-2xl bg-[rgb(var(--primary))] text-white">
              MP
            </span>
            Meal Planner
          </Link>
          ...
          >
            {navigation.map(([href, label]) => (
              <GhostLink key={href} href={href} active={pathname.startsWith(href)}>
                {label}
              </GhostLink>
            ))}
          </nav>
      ...
      <main className="container-page space-y-6">
        {error && (
          <div role="alert" className="rounded-xl bg-red-100 p-3 text-red-900">
            {error}
            {repo.capabilities.canReset
              ? ' Try again or reset dummy data in Settings.'
              : ' Try again or sign in again.'}
          </div>
        )}
        {message && (
          <div role="status" className="rounded-xl bg-emerald-100 p-3 text-emerald-900">
            {message}
          </div>
        )}
        {children}
