...
export function Button({ className = '', variant = 'primary', ...p }: ComponentProps<'button'> & {
    variant?: Variant;
}) {
...
export function ActionLink({ className = '', variant = 'primary', ...p }: ComponentProps<typeof Link> & {
    variant?: Variant;
}) {
...
export function GhostLink({ className = '', active = false, ...p }: ComponentProps<typeof Link> & {
    active?: boolean;
}) {
...
<div>{eyebrow && <p className="text-sm font-semibold uppercase tracking-wide text-[rgb(var(--primary))]">{eyebrow}</p>}<h1 className="mt-1 text-3xl font-bold tracking-tight md:text-4xl">{title}</h1>
{description && <p className="mt-2 max-w-2xl text-sm leading-6 text-[rgb(var(--muted))]">{description}</p>}</div>
{action}</div>;
}
...
export function Field({ label, description, className = '', ...p }: ComponentProps<'input'> & {
    label: string;
    description?: string;
}) { return <label className={`grid gap-1.5 text-sm font-semibold text-[rgb(var(--foreground))] ${className}`}>
<span>{label}{p.required && <span className="text-[rgb(var(--destructive))]"> *</span>}</span>
{description && <span className="text-xs font-normal text-[rgb(var(--muted))]">{description}</span>}<input {...p} className={inputClass}/>
</label>; }
export function Select({ label, children, className = '', ...p }: ComponentProps<'select'> & {
    label: string;
}) { return <label className={`grid gap-1.5 text-sm font-semibold text-[rgb(var(--foreground))] ${className}`}>
<span>{label}</span>
<select {...p} className={inputClass}>{children}</select>
</label>; }
export function Badge({ children, tone = 'neutral' }: {
    children: React.ReactNode;
    tone?: 'neutral' | 'success' | 'warning' | 'danger';
}) { const colors = { neutral: 'bg-[rgb(var(--primary-soft))] text-[rgb(var(--primary))]', success: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200', warning: 'bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200', danger: 'bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-200' }; return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${colors[tone]}`}>{children}</span>; }
export function Empty({ title, body }: {
    title: string;
    body: string;
}) { return <div className="rounded-[var(--radius)] border border-dashed border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-10 text-center">
<div className="mx-auto mb-3 grid size-12 place-items-center rounded-2xl bg-[rgb(var(--primary-soft))] text-xl">🍽️</div>
